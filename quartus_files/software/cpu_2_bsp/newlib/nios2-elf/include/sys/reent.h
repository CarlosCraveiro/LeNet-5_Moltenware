...
#ifdef WANT_SMALL_STDIO /* INTEL SMALL STDIO */     /* IntelSpecific */
/* We sort of cheat a lot here.
   Since we do no buffering, and don't support a whole bunch of
   stuff like ungetc, etc, we have a progressive FILE struct
   that grows (and essentially downcasts) based on its use. */
struct __sFILE_small_file {
  short	_flags;		/* flags, below; this FILE is free if 0 */
  short	_file;		/* fileno, if Unix descriptor, else -1 
                           (which makes it of type str) */
  int (*_sfvwrite) (struct _reent *, struct __sFILE_small_file *, _CONST char *, int); 
                        /* this gets set to the string handling
                           function (same justification as realloc ptr) */
};

struct __sFILE_small_str {
  short	_flags;		/* flags, below; this FILE is free if 0 */
  short	_file;		/* fileno, if Unix descriptor, else -1 */
  int (*_sfvwrite) (struct _reent *, struct __sFILE_small_str *, _CONST char *, int); 
                        /* this gets set to the string handling
                           function (same justification as realloc ptr) */
  struct __sbuf _bf;    /* bufer for things such as sprintf */
  unsigned char *_p;	/* current position in (some) buffer */
  int	_w;		/* write space left for putc() */
  _PTR (*_realloc_r) (struct _reent *, _PTR, int);  /* should be size_t */
                        /* if the __SMBF flag is set, this needs
                           to be a pointer to _realloc_r [1] */
};
/* [1] a pointer to malloc is used so that we don't
   pull in _realloc_r unless asprintf (or cousins) are 
   actually used */
#endif /* WANT_SMALL_STDIO */                       /* IntelSpecific */
...
# ifdef WANT_SMALL_STDIO /* INTEL SMALL STDIO */ /* IntelSpecific */
#  error "You must not use WANT_SMALL_STDIO with large files"
...
# ifdef WANT_SMALL_STDIO /* INTEL SMALL STDIO */ /* IntelSpecific */
typedef struct __sFILE_small_file    __FILE;     /* IntelSpecific */
# else /* !WANT_SMALL_STDIO */                   /* IntelSpecific */
...
struct _reent
{
  ...
  /* see comments about WANT_SMALL_STDIO in local.c */  /* IntelSpecific */
  #endif /* !WANT_SMALL_STDIO */                        /* IntelSpecific */
...
#ifdef _REENT_BACKWARD_BINARY_COMPAT
#ifdef WANT_SMALL_STDIO                     /* IntelSpecific */
...
#ifdef WANT_SMALL_STDIO                     /* IntelSpecific */
  struct __sFILE_small_file __sf_fake[3];   /* IntelSpecific */
#else /* !WANT_SMALL_STDIO */               /* IntelSpecific */
...
#ifdef WANT_SMALL_STDIO                     /* IntelSpecific -- entire section */

#define _REENT_INIT(var) \
  { 0, \
    (__FILE *)&var.__sf_fake[0], \
    (__FILE *)&var.__sf_fake[1], \
    (__FILE *)&var.__sf_fake[2], \
    _NULL, \
    NULL, \
    0, \
    0, \
    ...
    {{0x0004 /* should be __SRD */, 0, _NULL}, \
    {0x0008 | 0x0002 /* should be __SWR | __SNBF */, 1, _NULL}, \
    {0x0008 | 0x0002 /* should be __SWR | __SNBF */, 2, _NULL}}, \
    _NULL, \
    _NULL }

#ifdef _REENT_BACKWARD_BINARY_COMPAT /* IntelSpecific */
#define _REENT_INIT_PTR(var) \
  { \
    var->_stdin = (__FILE *)&var->__sf_fake[0]; \
    var->_stdout = (__FILE *)&var->__sf_fake[1]; \
    var->_stderr = (__FILE *)&var->__sf_fake[2]; \
    var->_errno = 0; \
    var->_mp = _NULL; \
    var->_locale = _NULL;\
    ...
    var->__sf_fake[0]._flags = 0x0004; /* should be __SRD */ \
    var->__sf_fake[0]._file = 0; \
    var->__sf_fake[0]._sfvwrite = _NULL; \
    var->__sf_fake[1]._flags = 0x0008 | 0x0002; /* should be __SWR | __SNBF */ \
    var->__sf_fake[1]._file = 1; \
    var->__sf_fake[1]._sfvwrite = _NULL; \
    var->__sf_fake[2]._flags = 0x0008 | 0x0002; /* should be __SWR | __SNBF */ \
    var->__sf_fake[2]._file = 2; \
    var->__sf_fake[2]._sfvwrite = _NULL; \
  }
#else /* !_REENT_BACKWARD_BINARY_COMPAT */ /* IntelSpecific */
#define _REENT_INIT_PTR(var) \
  { \
    var->_stdin = (__FILE *)&var->__sf_fake[0]; \
    var->_stdout = (__FILE *)&var->__sf_fake[1]; \
...
#endif /* !_REENT_BACKWARD_BINARY_COMPAT */ /* IntelSpecific */
...
void _reclaim_reent _PARAMS ((struct _reent *));  /* IntelSpecific */
